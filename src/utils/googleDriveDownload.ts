...
export class GoogleDriveDownloader {
  ...
  async downloadWithRetry(options?: {
    fileId?: string;
    fileName?: string;
    applyToSite?: boolean;
    maxRetries?: number;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.notifyProgress({
          stage: 'searching',
          message: attempt > 1 ? `Retry attempt ${attempt}/${maxRetries}...` : 'Starting download...',
          progress: 0
        });

        return await this.downloadContent(options);
      } catch (error) {
        lastError = error;
    ...
    this.notifyProgress({
      stage: 'error',
      message: `All ${maxRetries} download attempts failed: ${lastError.message}`,
      progress: 0
    });
  ...
  private autoSyncListeners: Array<(result: DownloadResult) => void> = [];
  ...
  startAutoSync(options?: {
    ...
    if (options?.onUpdate) {
      this.autoSyncListeners.push(options.onUpdate);
    }

    this.autoSyncInterval = setInterval(async () => {
      try {
        // Get last known timestamp
        const lastSync = localStorage.getItem('twofifths-last-google-drive-sync');
        const lastTimestamp = lastSync ? parseInt(lastSync) : 0;
        ...
        if (updateCheck.hasUpdates && updateCheck.fileInfo) {
          console.log('🔄 Auto-sync: Updates detected, downloading...');
          ...
          localStorage.setItem('twofifths-last-google-drive-sync', Date.now().toString());

          // Notify listeners
          this.autoSyncListeners.forEach(listener => listener(result));

          console.log('✅ Auto-sync: Content updated successfully');
        }
      } catch (error) {
    ...
    console.log(`🔄 Auto-sync started (checking every ${interval/1000} seconds)`);
  }
  ...
  onAutoSyncUpdate(listener: (result: DownloadResult) => void): () => void {
    this.autoSyncListeners.push(listener);
    return () => {
      this.autoSyncListeners = this.autoSyncListeners.filter(l => l !== listener);
    };
  }
}
